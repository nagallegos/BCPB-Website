...
async function initTemplatesCatalog() {
  ...
  try {
    ...
    allTemplates = (catalog.templates || []).map(normalizeTemplate);
  } catch (error) {
    console.error(error);
    countLabel.textContent = "The template catalog could not be loaded right now.";
  ...
  spotlightViewButton.addEventListener("click", () => {
    const featuredTemplate = getSpotlightTemplate(filteredTemplates, activeEventType);
    if (featuredTemplate) {
  ...
  function renderSpotlight(template) {
    if (!template) {
      ...
      spotlightDescription.textContent =
        "Publish templates from the admin catalog to populate this page.";
...
async function loadCatalog() {
  const sources = [
    "/.netlify/functions/get-template-catalog",
    "/data/published-templates.json",
  ];
  ...
  for (const source of sources) {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Catalog request failed with ${response.status} from ${source}`);
      }
...
function renderSummary(templates) {
  ...
  const eventTypes = new Set(
    templates.flatMap((template) => template.eventTypes),
  );
